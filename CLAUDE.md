...
- **Widgets and layouts** -- `PanelWidget`, `TableWidget`, `SparklineWidget`, `ChartWidget` (areas + lines, `▶` annotation, limit lines), `HeatmapWidget`, `StatTileWidget` (label/value/delta/gauge/trend, boxed into a row by `TileRowWidget`), `TimelineWidget`, `KeyHintBarWidget`, `LogoWidget` return `RenderedLine`s of known display width; `TerminalPalette` holds the shared styles (the status vocabulary, the threshold and response-time styles, the logo gradient endpoints and their interpolation). `LiveDashboardLayout`, `TestSelectionMenuLayout`, `LoadSummaryLayout`, `AdvancedTableLayout`, `StartupBanner` and `ExceptionRenderer` are pure functions of their inputs (no console, no clock), so identical inputs render identical frames. `LiveDashboardLayout`'s class comment is the spec for the frame -- the pieces, the width/height drop order, the column rule; read it before changing the dashboard.
- **Live view** -- `ConsoleManager` (`Internals/ConsoleOutput/`) runs one loop (250 ms tick, 1 Hz force-refreshed collector samples into `ScenarioLiveMetrics`) driving either `LiveDashboard` (alternate screen; `q` calls `TestExecutionState.RequestStop()`, the same path as Ctrl+C) or `LiveStatsWriter` (plain lines, zero escapes). `TestSelectionMenu`, `LiveViewDemo` and `LiveViewDemoScript` (`run --demo [--demo-duration=<seconds>]`) live in `StandaloneRunner/`.
- **View state** -- what the *viewer* chose, as opposed to what the run measured: the immutable `LiveDashboardViewState` (`LiveDashboardView` overview / step detail / error log, selected step by declaration index, error-log scroll, pause, time window, help) is owned by `ConsoleManager` and derived from a key press by the pure `LiveDashboardKeyHandler`; the layout is a function of the snapshots and one of these. Pausing freezes the snapshots the manager hands the dashboard, never the sampling.
- **Thresholds** -- `Internals/Thresholds/`: `ThresholdEvaluator` is the one place a `Threshold` is read and compared, live (per closed interval, into `LiveThreshold` + `ThresholdState`, driving the tile gauges and the chart limit lines) and as the completion verdict (`EvaluateVerdict` over the cumulative result, into `ThresholdResult`), so the two can never disagree. `ThresholdFormat` owns the labels, symbols and value formats shared by the dashboard, both summaries and `ThresholdViolationException`.
