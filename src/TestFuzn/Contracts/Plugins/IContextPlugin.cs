...
public interface IContextPlugin
{
    ...
    /// When <see langword="true"/>, <see cref="InitIteration"/> is called for each iteration
    /// and the resulting state is stored in <see cref="ContextPluginsState"/>.
    /// </summary>
    bool RequireIterationState { get; }
    ...
    object InitIteration(IServiceProvider serviceProvider);
    ...
    /// </summary>
    /// <param name="state">The plugin state created by <see cref="InitIteration"/>.</param>
    ...
    /// </summary>
    /// <param name="state">The plugin state created by <see cref="InitIteration"/>.</param>
    Task CleanupIteration(object state);
}