...
internal class LoadHtmlReportWriter : ILoadReport
{
    ...
    private string GenerateHtmlReport(LoadReportData loadReportData)
    {
        ...
        // Header
        builder.AppendLine($"<h1>Scenario: {loadReportData.ScenarioResult.ScenarioName}</h1>");
        ...
        builder.AppendLine($"<p>Init Started: {loadReportData.ScenarioResult.InitStartTime.ToLocalTime()}</p>");
        builder.AppendLine($"<p>Init Ended: {loadReportData.ScenarioResult.InitEndTime.ToLocalTime()}</p>");
        if (loadReportData.ScenarioResult.HasWarmupStep())
        { 
            builder.AppendLine($"<p>Warmup Started: {loadReportData.ScenarioResult.WarmupStartTime.ToLocalTime()}</p>");
            builder.AppendLine($"<p>Warmup Ended: {loadReportData.ScenarioResult.WarmupEndTime.ToLocalTime()}</p>");
        }
        builder.AppendLine($"<p>Execution Started: {loadReportData.ScenarioResult.MeasurementStartTime.ToLocalTime()}</p>");
        builder.AppendLine($"<p>Execution Ended: {loadReportData.ScenarioResult.MeasurementEndTime.ToLocalTime()}</p>");
        builder.AppendLine($"<p>Cleanup Started: {loadReportData.ScenarioResult.CleanupStartTime.ToLocalTime()}</p>");
        builder.AppendLine($"<p>Cleanup Ended: {loadReportData.ScenarioResult.CleanupEndTime.ToLocalTime()}</p>");
        builder.AppendLine($"<p>Total Run Duration: { loadReportData.ScenarioResult.TotalExecutionDuration.ToString("hh\\:mm\\:ss\\:ff")}</p>");
