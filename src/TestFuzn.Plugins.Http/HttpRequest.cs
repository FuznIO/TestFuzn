...
public class HttpRequest
{
    ...
    public async Task<HttpResponse> Send()
    {
        ...
        try
        {
            ...
            _context.Logger.LogInformation($"Step {_context.CurrentStep.Name} - HTTP Request: {request.Method} {request.RequestUri} - CorrelationId: {_context.CorrelationId}");

            if (request.Content != null)
            {
                var requestBody = await request.Content.ReadAsStringAsync(cts.Token);
                _context.Logger.LogInformation($"Step {_context.CurrentStep.Name} - Request Body: {requestBody} - CorrelationId: {_context.CorrelationId}");
            }

            response = await client.SendAsync(request, cts.Token);
            responseBody = await response.Content.ReadAsStringAsync(cts.Token);

            _context.Logger.LogInformation($"Step {_context.CurrentStep.Name} - HTTP Response: {(int)response.StatusCode} {response.ReasonPhrase} - CorrelationId: {_context.CorrelationId}");
            if (responseBody != null)
                _context.Logger.LogInformation($"Step {_context.CurrentStep.Name} - Response Body: {responseBody} - CorrelationId: {_context.CorrelationId}");
